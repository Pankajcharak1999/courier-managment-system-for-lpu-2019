#Pack frogot is used to hide the object that object is not destroyed
from tkinter import *
...
# Database 
with sqlite3.connect('lpu.db') as db:
    c = db.cursor()
try:
...
class main:
    ...
    def login(self):
        ...
        if result:
            ...
            ms.showerror('Oops!','Username Not Found.do you want to create a new user')
    ...
    def widgets(self):
    	#heading and all other detail and bd is border of heading & RELIEF is saying type of box 
        Top = Frame(root, bd=20,  relief=RIDGE)
        Top.pack(side=TOP, fill=X)
        self.head = Label(Top, text = "Python: Courier management system for lpu", font=('arial', 15))
        self.head.pack(fill=X)
        self.hLogin:ead = Label(self.master,text = 'LOGIN',font = ('',20),pady = 10)
        self.head.pack()
        #login page
...
if __name__ == '__main__':
    ...
    root = Tk()
    root.title("Python: Courier management system for lpu")
    main(root)
    
    canvas = Canvas(root)
    canvas.pack()
    canvas.config(width = 640, height = 100)
    root.mainloop()
